...
real :: a(*),b(*)
integer :: itype,ngrd
!JCL(03/17/2004) make flng longer to accommodate long pathnames in *job* file
!character*(*) flnm,cgrid*1,flng*80,errmsg*120,string
character*(*) flnm,cgrid*1,flng*200,errmsg*120,string
logical there
...
do ni=1,nvbtab
   ...
   if(string.eq.anal_table(ni)%string.and.ngrd.eq.anal_table(ni)%ngrid) then
      ...
      call RAMS_c_open(flng(:lastchar(flng))//char(0),'r'//char(0))
!      print*,' C_open - ',flng(1:lastchar(flng)),'   ',string
