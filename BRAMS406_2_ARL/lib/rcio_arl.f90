...
SUBROUTINE COMMIO (CFILE,IO,IUN,igrd,nxrams,nyrams,nzrams, &
                   dxrams,xmom,ymom,xtmp,ytmp,plon,plat,ztk,zmk)
use somevars

CHARACTER*(*) IO,CFILE
include 'rcommons.h'

real, dimension(80) :: ztk
!JCL(3/27/03) add variable to store altitudes of FLUX (not scalar) LEVELS
real, dimension(80) :: zmk

!     This routine reads or writes the history and analysis file common blocks.

integer cio_i,cio_f,cio_f8,cio_c
real xmom(nxpmax),ymom(nypmax),xtmp(nxpmax),ytmp(nypmax)
character cng*2
...
ie=cio_i(iun,irw,'nclouds',nclouds,1)
if (ie /= 0) nclouds=1
...
call alloc_somevars(myngrids,myn3)
!ML]
...
1       format(i2.2)
   ...
   ie=cio_f(iun,irw,'rt01dn'//cng,rt01dn(1,ng),nnzp(ng))
     mydzmn(1:nnzp(ng),ng)=dzmn(1:nnzp(ng),ng)
     mydztn(1:nnzp(ng),ng)=dztn(1:nnzp(ng),ng)
enddo
...
ie=cio_i(iun,irw,'inucprg',inucprg,1)
...
! sbkim: zt for comp_ARL.f90
do k=1,nnzp(igrd)
  ztk(k) = ztn(k,igrd)
!JCL(3/27/03): add variable that stores flux levels as well
  zmk(k) = zmn(k,igrd)
enddo
