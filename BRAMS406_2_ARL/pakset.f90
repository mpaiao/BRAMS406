...
!-------------------------------------------------------------------------------
  ...
  INTEGER  :: ntot,krec,k,i,nlvl,kg,ng,nv,l,nl,nvar 
...
!     READ(LUNIT,'(20X,F6.0,I3,99(1X,A4))')                                 &
     READ(LUNIT,'(20X,F7.1,I3,99(1X,A4))')                                 &
...
!==>compute extended header length
  ...
  DO L=1,NLVL
     ...
     NVAR=GV(NG)%NVAR(L)
     DO K=1,NVAR
!CHG (10/02/2003) from 8 to 16 bit
!        GV(NG)%LENH=GV(NG)%LENH+8
        GV(NG)%LENH=GV(NG)%LENH+10
     END DO
  END DO

!==>check for multiple extended header records

!CHG (10/02/03) use 16 bits
!  GV(NG)%LREC=(GV(NG)%NXG*GV(NG)%NYG)
  GV(NG)%LREC=(GV(NG)%NXG*GV(NG)%NYG)*2
! check limits
  IF((GV(NG)%LENH).GT.MLEN.OR.(GV(NG)%LREC).LT.110)THEN
