...
subroutine commio_ncep(ngrid,ntimes)
   ...
   use mod_ncdf_globio, only : ncio_glo,ncio_glo_sca, ncio_dim, ncio_1dnotime,ncio_ncep_time
...
!------ Giving the preprocessor the option to entirely skip this routine ------------------!
   ...
   !----- NCEP will have always a single grid ---------------------------------------------!
   ngrid  = 1
   ngrids = 1
   ...
   !----- NCEP will always have a regular lon/lat grid ------------------------------------!
   ihtran = 0
   !----- NCEP will always have a regular pressure coordinate -----------------------------!
   if_adap = 4
   ...
   ierr   = ncio_1dnotime('level'         ,.true.,nnzp(ngrid), ztn(1:nnzp(ngrid),ngrid))
