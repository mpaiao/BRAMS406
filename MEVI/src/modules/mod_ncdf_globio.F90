!==========================================================================================!
!==========================================================================================!
! MEVI. Module mod_ncdf_globio. This module contains two functions that handle reading      !
...
!------------------------------------------------------------------------------------------!
   ...
   integer function ncio_wrf_time(varname,crashmissing,ntimes,timelength,timeout)
      ...
      integer          , external         :: julday,v5d_datestamp,v5d_timestamp,julday1583
         ...
         !----- Guess it's okay to get the variable now -----------------------------------!
            ...
            do t=1,ntimes
               ...
               timeout(t)%gradsstamp = grads_dtstamp(timeout(t)%year,timeout(t)%mmm        &
                                                    ...
                                                    ,timeout(t)%minu)
               timeout(t)%timestr    = mevi_dtstamp(timeout(t)%year,timeout(t)%month       &
                                                   ,timeout(t)%day,timeout(t)%hour         &
                                                   ,timeout(t)%minu,timeout(t)%seco)
               timeout(t)%elapsed   = dble(julday1583(timeout(t)%month,timeout(t)%day      &
                                                     ,timeout(t)%year))                    &
   ...
   !=======================================================================================!
   !   This function retrieves the time variable with a single dimension. This should be   !
   ! used to retrieve ECMWF time based on any time-variant variable, because time is one   !
   ! attribute of every variable, not a variable on its own.                               !
   !---------------------------------------------------------------------------------------!
   integer function ncio_eranat_time(varname,crashmissing,ntimes,timeout)
      use netcdf
      ...
      !----- 2. Guess it's okay, now I need to check whether other problems exist. --------!
         ...
         elseif (xtype /= NF90_FLOAT) then
            call fatal_error('Variable '//trim(varname)//' is not real!'                   &
                            ,'ncio_erainit_time','mod_ncdf_globio.F90')
         elseif (ndims /= 3) then !--- Time should be the only time -----------------------!
            call fatal_error(                                                              &
              'ncio_eranat_time downloads only 3D variables; '//trim(varname)//' is not!'   &
             ,'ncio_ecmwf_time','mod_ncdf_globio.F90')
         !----- Guess it's okay to get the variable now -----------------------------------!
         else

            ierr = nf90_get_att(ncid,varid,'initial_time',tmpvar)
            if (ierr /= NF90_NOERR) then
               call ncdf_load_err(ierr)
               call fatal_error ('Error retrieving the variable '//trim(varname)//'...'    &
                                ,'ncio_ecmwf_time','mod_ncdf_globio.F90')
            ...
            read(tmpvar,fmt='(2(i2,1x),i4,2x,i2,1x,i2)') timeout(1)%month                  &
                                                        ,timeout(1)%day                    &
                                                        ,timeout(1)%year                   &
                                                        ,timeout(1)%hour                   &
                                                        ,timeout(1)%minu
            write(unit=*,fmt='(a,1x,2(i2.2,a),i4.4,1x,i2.2,a,i2.2,1x,a)')                  &
              '         [|] Retrieving time...'                                            &
             ,timeout(1)%month,'-',timeout(1)%day,'-',timeout(1)%year                      &
             ,timeout(1)%hour,':',timeout(1)%minu,'UTC'

            timeout(1)%seco    = 0
            timeout(1)%doy     = julday(timeout(1)%month,timeout(1)%day,timeout(1)%year)
            timeout(1)%mmm     = monchar(timeout(1)%month)
            timeout(1)%fracday = day_fraction(timeout(1)%hour,timeout(1)%minu              &
                                             ,timeout(1)%seco)
            timeout(1)%yyyyddd    = v5d_datestamp(timeout(1)%year,timeout(1)%doy)
            timeout(1)%gradsstamp = grads_dtstamp(timeout(1)%year,timeout(1)%mmm           &
                                                 ,timeout(1)%day,timeout(1)%hour           &
                                                 ,timeout(1)%minu)
            timeout(1)%timestr    = mevi_dtstamp(timeout(1)%year,timeout(1)%month          &
                                                ,timeout(1)%day,timeout(1)%hour            &
                                                ,timeout(1)%minu,timeout(1)%seco)
            timeout(1)%elapsed   = dble(julday1583(timeout(1)%month,timeout(1)%day         &
                                                  ,timeout(1)%year))                       &
                                 + dble(timeout(1)%fracday)
         end if
      end if

      ncio_eranat_time = ierr

      return
   end function ncio_eranat_time
   !=======================================================================================!
   !=======================================================================================!
   ...
   !=======================================================================================!
   !=======================================================================================!
   !   This function retrieves a real variable with rank 2 and does not depend on time.    !
   !---------------------------------------------------------------------------------------!
   integer function ncio_2dvarnotime(varname,crashmissing,dim1,dim2,realout,offin1,offin2  &
                                    ,strin1,strin2)
      use netcdf
      use mod_ioopts, only : missflg_real ! ! intent(in)
      use mod_netcdf, only : ncid         & ! intent(inout)
                           , varid        & ! intent(inout)
                           , dummy_vname  & ! intent(inout)
                           , dimid        & ! intent(inout)
                           , xtype        & ! intent(inout)
                           , ndims        & ! intent(inout)
                           , dimids       & ! intent(inout)
      ...
      integer                   , intent(in), optional :: offin1
      integer                   , intent(in), optional :: offin2
      integer                   , intent(in), optional :: strin1
      integer                   , intent(in), optional :: strin2
      !----- Optional output, only one of them should appear there ------------------------!
      ...
      if (ierr /= NF90_NOERR .and. crashmissing) then
         ...
         call fatal_error ('2D-Variable  '//trim(varname)//' wasn''t found!!!'             &
                          ,'ncio_2dvarnotime','mod_ncdf_globio.F90')
      !----- 2. Guess it's okay, now I need to check whether other problems exist. --------!
      else if (ierr /= NF90_NOERR .and. (.not. crashmissing)) then
         continue
      else
         ...
         elseif (ndims /= 2) then !--- Input variable ought to be 2D... -------------------!
            call fatal_error('ncio_2dvarnotime downloads only 2D variables and '//         &
                             trim(varname)//' is not!','ncio_2dvarnotime'                  &
                            ,'mod_ncdf_globio.F90')
         !----- Guess it's okay to get the variable now -----------------------------------!
         else
            ierr = nf90_get_var(ncid,varid,realout,start=start,count=count,stride=stride)
      ...
      ncio_2dvarnotime = ierr

      return
   end function ncio_2dvarnotime
   !=======================================================================================!
   !=======================================================================================!
   ...
   !=======================================================================================!
   !=======================================================================================!
   !   This function retrieves a real variable with rank 3 (3D variables with no           !
   ! associated time.                                                                      !
   !---------------------------------------------------------------------------------------!
   integer function ncio_3dvarnotime(varname,crashmissing,dim1,dim2,dim3,realout,offin1    &
      ...
      integer                           , intent(in), optional :: offin1
      integer                           , intent(in), optional :: offin2
      integer                           , intent(in), optional :: offin3
      integer                           , intent(in), optional :: strin1
      integer                           , intent(in), optional :: strin2
      integer                           , intent(in), optional :: strin3
      !----- Optional output, only one of them should appear there ------------------------!
      ...
      !----- Defining the dimensions so it gets only 1 time -------------------------------!
      start   = (/ 1+off1, 1+off2, 1+off3 /)
      count   = (/   dim1,   dim2,   dim3 /)
      stride  = (/   str1,   str2,   str3 /)
      !----- Checking where the variable I want to retrieve is ----------------------------!
      ierr = nf90_inq_varid(ncid,varname,varid)
      ...
      if (ierr /= NF90_NOERR .and. crashmissing) then
         ...
         call fatal_error ('3D-Variable  '//trim(varname)//' wasn''t found!!!'             &
                          ,'ncio_3dvarnotime','mod_ncdf_globio.F90')
      !----- 2. Guess it's okay, now I need to check whether other problems exist. --------!
      else if (ierr /= NF90_NOERR .and. (.not. crashmissing)) then
         continue
      else
         ...
         elseif (xtype /= NF90_FLOAT) then
            call fatal_error('Variable '//trim(varname)//' is not real!'                   &
                            ,'ncio_3dvarnotime','mod_ncdf_globio.F90')
         elseif (ndims /= 3) then !--- Variable ought to be 3-D... ------------------------!
            call fatal_error('ncio_3dvarnotime downloads only 3D variables and '//         &
                             trim(varname)//' is not!','ncio_3dvarnotime'                  &
                            ,'mod_ncdf_globio.F90')
         !----- Guess it's okay to get the variable now -----------------------------------!
         else
            ierr = nf90_get_var(ncid,varid,realout,start=start,count=count,stride=stride)
      ...
      ncio_3dvarnotime = ierr

      return
   end function ncio_3dvarnotime
   !=======================================================================================!
   !=======================================================================================!
   ...
   integer function ncio_3dvar(varname,crashmissing,it,dim1,dim2,dim3,realout,offin1       &
      ...
      !----- Check whether the optional input variables are present. ----------------------!
      off1 = 0
      off2 = 0
      off3 = 0
      if (present(offin1)) off1 = offin1
      ...
      !----- Define the dimensions so it gets only one time. ------------------------------!
      start   = (/ 1+off1, 1+off2, 1+off3,   it /)
      count   = (/   dim1,   dim2,   dim3,    1 /)
      stride  = (/   str1,   str2,   str3,    1 /)
      !----- Check where the variable we want to retrieve is. -----------------------------!
      ierr = nf90_inq_varid(ncid,varname,varid)

      !------------------------------------------------------------------------------------!
      !     Now we will check for possible common errors, and decide how to handle them.   !
      !------------------------------------------------------------------------------------!
      !----- 1. Variable is non-optional and it is not there. -----------------------------!
      if (ierr /= NF90_NOERR .and. crashmissing) then
         call ncdf_load_err(ierr)
      ...
      !----- 2. Guess it's fine, now I need to check whether other problems exist. --------!
      else if (ierr /= NF90_NOERR .and. (.not. crashmissing)) then
         continue
      else
         ierr = nf90_inquire_variable(ncid,varid,dummy_vname,xtype,ndims,dimids,natts)
         !----- Now we check several possible problems that will keep us to get the data --!
         if (ierr /= NF90_NOERR .and. crashmissing) then
            call ncdf_load_err(ierr)
            call  fatal_error('Failed retrieving dimension '//trim(varname)//'!!!'         &
                             ,'ncio_3dvar','mod_ncdf_globio.F90')
         ...
         !----- Guess it's fine to get the variable now -----------------------------------!
         else
            ierr = nf90_get_var(ncid,varid,realout,start=start,count=count,stride=stride)
            write (unit=*,fmt='(2(a,1x),2a,2(1x,es14.7))')                                 &
                  '         [|] Retrieving :',trim(varname),'...'                          &
